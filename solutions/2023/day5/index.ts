				...
				<const>[
					...
					...other.map((category) =>
						category
							.split("\n")
							.slice(1)
							.map((line) => [...line.matchAll(/\d+/g)].map((match) => parseInt(match[0])) as [number, number, number])
							.sort((a, b) => a[1] - b[1])
					),
				]
		)
		.use(([seeds, ...categories]) =>
			categories.splice(0, 9).reduce(
				(prev, category) =>
					category
						.reduce((acc, [_, src, len]) => (acc.includes(src) ? [...acc, src + len] : [...acc, src, src + len]), [0])
						.reduce(
							(acc, src) =>
								acc.flatMap(([start, end]) =>
									src > start && src < end
										? [
												[start, src - 1],
												[src, end],
						...
						.use((splitted) =>
							category.reduce(
								(acc, [dst, src, len]) =>
									acc.map(([start, end]) =>
										start >= src && end <= src + len ? [start + (dst - src), end + (dst - src)] : [start, end]
									),
								splitted
							)
						),
