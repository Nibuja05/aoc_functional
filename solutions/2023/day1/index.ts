		...
		.map((line) =>
			[...line.matchAll(/(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine)|(\d)/g)]
				.flatMap((m) =>
					m
						.map((match, groupIndex, arr) =>
							match && groupIndex
								? groupIndex > 9
									? [parseInt(match), (arr as MatchArr)["index"]]
				...
				.map((elem, index, arr) =>
					index < arr.length - 1
						? elem![0]
						: [...line.substring(elem![1] + 1).matchAll(/(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine)/g)]
								.flatMap((m) => m.map((match, groupIndex) => (match && groupIndex ? groupIndex : null)).filter((index) => index))
								.use((result) => (result.length == 0 ? elem![0] : result[0]))
				)
		)
		.map((numbers) => [numbers[0]!, numbers[numbers.length - 1]!])
		.map(([n1, n2]) => parseInt(`${n1}${n2}`))
