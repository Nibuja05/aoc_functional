		...
		.map((line) =>
			line
				...
				.use(
					([hand, bid]) =>
						<const>[
							hand.split("").map((char) => (/\d/.test(char) ? parseInt(char) : { T: "a", J: "b", Q: "c", K: "d", A: "e" }[char]!)),
							parseInt(bid),
						]
				)
		)
		...
		.map((line) =>
			line
				...
				.use(
					([hand, bid]) =>
						<const>[
							hand.split("").map((char) => (/\d/.test(char) ? parseInt(char) : { T: "a", J: 0, Q: "c", K: "d", A: "e" }[char]!)),
							parseInt(bid),
						]
				)
		)
		.map(
			([hand, bid]) =>
				<const>[
					...
					bid,
					hand
						.reduce((acc, num) => ((acc[num] = (acc[num] || 0) + 1), acc), {} as Record<string, number>)
						.use((occurances) =>
							Object.entries(occurances)
								...
								.use(([win1, win2]) =>
									(win1 ?? 0) + (occurances[0] ?? 0) == 5
										? 6
										: (win1 ?? 0) + (occurances[0] ?? 0) == 4
										? 5
										: (win1 ?? 0) + (occurances[0] ?? 0) == 3 && (win2 ?? 0) == 2
										? 4
										: (win1 ?? 0) + (occurances[0] ?? 0) == 3
										? 3
										: (win1 ?? 0) + (occurances[0] ?? 0) == 2 && (win2 ?? 0) == 2
										? 2
										: (win1 ?? 0) + (occurances[0] ?? 0) == 2
										? 1
										: 0
								)
						),
