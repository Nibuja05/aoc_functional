		...
		.map(
			(line) =>
				<const>[
					...
					[...line.matchAll(/\d+/g)].map((match) => <const>[match.index!, match.index! + match[0].length, parseInt(match[0])]),
				]
		)
			...
			numbers
				.filter(
					([start, end, _]) =>
						...
						(lineIndex > 0 && lines[lineIndex - 1][0].some((symbol) => symbol >= start - 1 && symbol <= end)) ||
						(lineIndex < lines.length - 1 && lines[lineIndex + 1][0].some((symbol) => symbol >= start - 1 && symbol <= end))
				)
...
console.log(
	getInput()
		...
		.flatMap(([symbols, numbers], lineIndex, lines) =>
			symbols
				.map((index) => [
					...numbers.filter(([start, end, _]) => start == index + 1 || end == index).map(([_, __, number]) => number),
					...(lineIndex == 0
						? []
						: lines[lineIndex - 1][1]
